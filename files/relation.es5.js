...
(function () {
  ...
  chart.prototype.init = function () {
    ...
    var simulation = d3.forceSimulation(nodes).force("charge", d3.forceManyBody().strength(-400)).force("link", d3.forceLink(edges).distance(200).strength(2)).force("center", d3.forceCenter(width / 2, height / 2)).force("collide", d3.forceCollide().radius(function (d) {
      return d.r + 50;
    }));
    ...
    //nodes g
    var gs = g.selectAll(".circle").data(nodes).enter().append("g").attr("class", "gNodes").attr("transform", "translate(-9999, -9999)").call(d3.drag().on("start", dragstart).on("drag", dragged).on("end", dragend));
    ...
    gs.on("click", function (d) {
      ...
      d3.selectAll("circle").attr("stroke-width", function (o) {
    ...
    //nodes circle
    var nodesCircle = gs.append("circle").attr("r", function (d) {
      return d.r - strokeWidth;
    }).attr("id", function (d) {
    ...
    //nodes text
    var text = gs.append("text").attr("id", "text").style("fill", "#fff").style("cursor", "pointer");
    ...
    text.append("tspan").attr("x", function (d) {
    ...
    function appendArrow(id, _class) {
      defs.append("marker").attr("class", "arrow").attr("markerUnits", "strokeWidth").attr("markerWidth", 10).attr("markerHeight", 10).attr("refY", 3).attr("id", "arrow" + _class).attr("fill-opacity", .5).append("path").attr("d", "M0,1 L0,5 L5,3 z");
    }
    ...
    function initPathRect(direction) {
      ...
      var pathLength = document.querySelector(".path-middle").getBBox().width;
      var rect = g.selectAll(".path-rect").data(edges).enter().append("rect").attr("fill", "none").attr("width", document.querySelector(".path-middle").getBBox().width).attr("height", _this.options.spacing);
      return rect;
    }
    ...
    function appendText(direction, arr, isMask) {
      ...
      var text = g.selectAll(selection).data(arr).enter().append("text").attr("id", function (d, i) {
        return direction + "Text" + i;
      }).attr("dy", 3);
      isMask ? text.attr("class", "path-" + direction + "-mask-text") : text.attr("class", "path-" + direction + "-text");
      return text;
    }

    function appendTextPath(direction, mask) {
      var idSelection = "#" + direction + "Path";
      mask.append("textPath").attr("xlink:href", function (d, i) {
        return "#d" + d.index;
      }).attr("id", "text").text(function (d, i) {
        var type = {
    ...
    function initPathTextMask(direction, arr) {
      ...
      appendTextPath(direction, mask).style("stroke", "white").style("stroke-width", "0.15em");
      return mask;
    }
    ...
    function initPath(direction, arr) {
      var attrClass = "path-" + direction,
        ...
        selection = "." + attrId,
        links = g.selectAll(selection).data(arr).enter().append("path").attr("class", attrClass).attr("id", function (d) {
          return "d" + d.index;
        });
      return links;
    }
    ...
    function fadePath(opacity, d, link, direction, state) {
      link.style("stroke-opacity", function (o, i) {
        if (o.source === d || o.target === d) {
          ...
          //不关联的link
          var arrowOpacity = state === "end" ? .5 : opacity;
      ...
      nodesCircle.style("stroke-opacity", function (o) {
        if (!isConnect(d, o)) {
          ...
          if (o.properties.name === _this.options.dominant) {
            ...
            o.color === _this.options.color.node.Company ? this.setAttribute("fill", _this.options.color.opacityNode.Company) : this.setAttribute("fill", _this.options.color.opacityNode.Person);
            o.stroke === _this.options.color.stroke.Company ? this.setAttribute("stroke", _this.options.color.opacityStroke.Company) : this.setAttribute("stroke", _this.options.color.opacityStroke.Person);
          }
    ...
    function sticky(_class, d) {
      d3.selectAll(_class).filter(function (o) {
        return o.source.index !== d.index || o.target.index !== d.index;
      }).lower();
    }
    ...
    function dragged(d) {
      ...
      $(".unSelected").parent("g").each(function (i, v) {
        d3.select(v).lower();
      });
    ...
    function textFixedTransform(direction) {
      return function (d, i) {
        ...
        var x = svgDom.x;
        var y = svgDom.y;
        var rotate0 = "rotate(0, " + (x + svgDom.width / 2) + ", " + (y + svgDom.height / 2) + ")";
        var rotate180 = "rotate(180, " + (x + svgDom.width / 2) + ", " + (y + svgDom.height / 2) + ")";
        var rotate = -90 < d.x && d.x < 90 ? direction === "middle" ? rotate0 : rotate180 : direction === "middle" ? rotate180 : rotate0;
        return rotate;
      };
    }

    function updateText(text, direction) {
      text.attr("x", textFixedX(direction)).attr("transform", textFixedTransform(direction));
    }
    ...
    function updatePath(path, direction, isDrag) {
      ...
      if (direction !== MIDDLE) {
        path.attr("marker-end", function (d, i) {
          ...
          arrow.attr("fill", _this.options.color.line[d.type]).attr("refX", 26).attr("orient", d.x);
          return "url(#arrow" + d.index + ")";
        });
      } else {
        path.attr("marker-end", function (d, i) {
          var arrow = d3.select("#arrow" + d.index);
          arrow.attr("fill", _this.options.color.line[d.type]).attr("refX", 28).attr("orient", "auto");
          return "url(#arrow" + d.index + ")";
        });
      }
    }
